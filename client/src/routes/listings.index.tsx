...
function ListingsPage() {
  ...
  useEffect(() => {
    if (lat !== null && lng !== null && (search.lat !== lat || search.lng !== lng)) {
      navigate({ search: (prev) => ({ ...prev, lat, lng, radiusKm: ((prev as typeof search).radiusKm ?? 10), page: 1 }) })
    }
    ...
    <main className="page-wrap px-4 pb-16 pt-8">
        ...
        <CardContent className="p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              style={!search.categoryId
                ? { background: 'var(--brand)', color: '#fff', borderColor: 'var(--brand)' }
                : { background: 'var(--warm-white)', color: 'var(--text-soft)', borderColor: 'var(--line)' }}
            >
              All
            </button>
            {cats.map((c) => (
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!search.lat ? (
              ...
              <>
                ...
                <Button variant="ghost" size="sm" onClick={clearGeo} className="h-7 gap-1 rounded-full text-xs">
      ...
      {hasFilters && (
        <div className="mb-4 flex flex-wrap items-center gap-2">
          ...
          {activeFilters.map((f) => (
            ...
            >
              {f.label}
              <button type="button" onClick={f.onRemove} className="opacity-70 hover:opacity-100" aria-label={`Remove ${f.label}`}>
                <X className="h-3 w-3" />
              </button>
            </Badge>
          ))}
          <Button variant="ghost" size="sm" onClick={clearAll} className="h-6 rounded-full text-xs">Clear all</Button>
        </div>
      )}
      ...
      {result.data.length === 0 ? (
        <Card className="border-0 p-12 text-center" style={{ background: 'var(--surface-strong)', border: '1px solid var(--line)' }}>
          ...
          {hasFilters && (
            <Button variant="link" onClick={clearAll} className="mt-3">Clear all filters →</Button>
          )}
        </Card>
      ) : (
        <>
          <div className="mb-4 flex items-center justify-between">
            ...
            {search.lat && (
              <Badge variant="secondary" className="gap-1 rounded-full text-xs" style={{ background: 'rgba(139,69,19,0.08)', color: 'var(--brand)' }}>
                📡 Sorted by distance
              </Badge>
            )}
          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {result.data.map((item) => {
              ...
              return (
                <Link key={item.id} to="/listings/$id" params={{ id: item.id }} className="group block no-underline">
                  <Card className="feature-card overflow-hidden border-0">
                    <div className="relative overflow-hidden" style={{ aspectRatio: '4/3', background: 'var(--sand)' }}>
                      ...
                      {search.lat && (
                        <Badge className="absolute right-2.5 top-2.5 text-[10px]" style={{ background: 'rgba(139,69,19,0.85)' }}>
                          📍 Nearby
                        </Badge>
                      )}
                    </div>
                      ...
                      <div className="flex items-end justify-between">
                        <div>
                          ...
                          {item.securityDeposit && Number(item.securityDeposit) > 0 && (
                            <p className="text-xs" style={{ color: 'var(--text-soft)' }}>Deposit ₹{Number(item.securityDeposit).toLocaleString('en-IN')}</p>
                          )}
                        </div>
          ...
          {result.meta.pages > 1 && (
            <div className="mt-8 flex flex-wrap justify-center gap-2">
              <Button variant="secondary" size="sm" onClick={() => push({ page: Math.max(1, search.page - 1) })} disabled={search.page <= 1}>
                ← Prev
              </Button>
                ...
                .map((p, i) =>
                  p === '…' ? (
                    ...
                    <Button
                      key={p}
                      size="sm"
                      variant={p === search.page ? 'default' : 'secondary'}
              ...
              <Button variant="secondary" size="sm" onClick={() => push({ page: Math.min(result.meta.pages, search.page + 1) })} disabled={search.page >= result.meta.pages}>
                Next →
              </Button>
            </div>
          )}
